...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level)
	{
		...
		return count += this.my_left_tree().my_count_at_level(level - 1) + this.my_right_tree().my_count_at_level(level - 1); // Counter is appended as it moves through the call stack
	}
	...
	public boolean my_is_balanced(){
		if(this.my_is_empty())
		return true; // base case. Every previous sub-tree has passed the test.
					
		int height_difference = Math.abs(this.my_left_tree().my_length() - this.my_right_tree().my_length());
					
		if(height_difference <= 1)
		return this.my_left_tree().my_is_balanced() && this.my_right_tree().my_is_balanced();
		
		return false; // Breaks immediately if failed
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		{
			T1 root_key_value = this.my_root().getKey();
			
			if(root_key_value.compareTo(key) < 0 )
				return smaller_counter += 1 + this.my_left_tree().my_count_smaller_nodes(key) + this.my_right_tree().my_count_smaller_nodes(key); 
			
			return smaller_counter = this.my_left_tree().my_count_smaller_nodes(key) + this.my_right_tree().my_count_smaller_nodes(key); // +1 is not added as the value is greater than or equal to the given key
		}
	}	
	...
	public int my_find_node_at_level(T1 key){
		...
		if(this.my_is_empty())
			return -1; // Base case, node is not found.
		
		T1 root_key = this.my_root().getKey();
		
		// Greater nodes placed on right of tree, smaller nodes placed left of tree
		if(key.compareTo(root_key) < 0)
			level_counter = this.my_left_tree().my_find_node_at_level(key);
		
		else if(key.compareTo(root_key) > 0)
			level_counter = this.my_right_tree().my_find_node_at_level(key);	
		

		if(level_counter == -1) // Not found after reaching the end
			return -1;
		
		return level_counter + 1; // Else return the found level + 1
	}		
}
